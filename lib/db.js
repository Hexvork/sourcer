...
function initDB() {
  ...
  if (!resumeCols.includes('birth_date')) db.exec("ALTER TABLE resumes ADD COLUMN birth_date TEXT DEFAULT ''");
  if (!resumeCols.includes('company')) db.exec("ALTER TABLE resumes ADD COLUMN company TEXT DEFAULT ''");
  // needs_ai：规则匹配不到（姓名/职位占位、缺出生日期等），等待 AI 配置后补全并重命名
  if (!resumeCols.includes('needs_ai')) db.exec('ALTER TABLE resumes ADD COLUMN needs_ai INTEGER DEFAULT 0');
  return db;
}
...
function getResumeByPath(filePath) {
  const d = getDB();
  return d.prepare('SELECT * FROM resumes WHERE original_path = ? ORDER BY id DESC LIMIT 1').get(filePath);
}

// 简历原文件被重命名后同步路径字段（original_path / source_file / pool_path）
function updateResumePath(id, newPath) {
  const d = getDB();
  const base = path.basename(newPath);
  d.prepare("UPDATE resumes SET original_path = ?, source_file = ?, pool_path = ?, updated_at = datetime('now','localtime') WHERE id = ?")
    .run(newPath, base, newPath, id);
}
...
function deleteProcessedFile(filePath) {
  const d = getDB();
  return d.prepare('DELETE FROM processed_files WHERE path = ?').run(filePath);
}

// ---------- 问题文件登记表（未知/异常/待 AI 修正） ----------
function upsertProblemFile(filePath, reason, status = 'pending') {
  const d = getDB();
  d.prepare(`INSERT INTO problem_files (file_path, original_name, reason, status, updated_at)
             VALUES (?, ?, ?, ?, datetime('now','localtime'))
             ON CONFLICT(file_path) DO UPDATE SET
               original_name=excluded.original_name,
               reason=excluded.reason,
               status=excluded.status,
               ai_attempts=problem_files.ai_attempts+1,
               updated_at=excluded.updated_at`)
    .run(filePath, path.basename(filePath), reason || '', status);
}

function removeProblemFile(filePath) {
  const d = getDB();
  return d.prepare('DELETE FROM problem_files WHERE file_path = ?').run(filePath);
}

function listProblemFiles() {
  const d = getDB();
  return d.prepare('SELECT * FROM problem_files ORDER BY updated_at DESC').all();
}
...
function setAppSetting(key, value) {
  const d = getDB();
  d.prepare(`INSERT INTO app_settings (key, value, updated_at) VALUES (?, ?, datetime('now','localtime'))
