...
const UNKNOWN_DIR = path.join(RESUME_DIR, '未知');
...
// 粗略判断内容是否像简历：没有这些特征的多半是资料/试题/非简历文档
function looksLikeResume(text) {
  const t = String(text || '');
  const indicators = [
    /姓名|名字/i, /性别/i, /年龄|\d+\s*岁/i, /学历|教育|毕业/i,
    /工作经历|工作内容|任职|就职/i, /求职意向|应聘|期望岗位/i,
    /技能|掌握|熟练/i, /项目经历|项目经验/i, /联系电话|手机|电话/i,
    /邮箱|E-?mail/i, /籍贯|户籍|民族/i, /自我评价/i, /个人简介|个人概述/i
  ];
  return indicators.some(re => re.test(t));
}

// 文件名本身是否像简历（防止把真简历误判成非简历搬走）
function looksLikeResumeFilename(filename) {
  const base = String(filename || '');
  if (/简历|resume|cv/i.test(base)) return true;
  // 含常见姓氏 + 2~4 字中文名，且旁边有岗位词或年份
  const surname = /(李|王|张|刘|陈|杨|赵|黄|周|吴|徐|孙|胡|朱|高|林|何|郭|马|罗|梁|宋|郑|谢|韩|唐|冯|于|董|萧|程|曹|袁|邓|许|傅|沈|曾|彭|吕|苏|卢|蒋|蔡|贾|丁|魏|薛|叶|阎|余|潘|杜|戴|夏|钟|汪|田|任|姜|范|方|石|姚|谭|廖|邹|熊|金|陆|郝|孔|白|崔|康|毛|邱|秦|江|史|顾|侯|邵|孟|龙|万|段|钱|汤|尹|黎|易|常|武|乔|贺|赖|龚|文|欧|区|覃|冼|韦|邝|麦|蓝|司)/;
  if (surname.test(base)) return true;
  return false;
}
...
function listResumeFiles(dir = RESUME_DIR) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (entry.name === '未知') continue; // 非简历隔离文件夹不参与扫描
...
function fallbackCategorize(text, filename) {
  ...
  const name = pickRegex(t, /(?:姓名|名字)[\s:：]{1,}([^\s,，。；;\n]{2,20}?)(?=\s*(?:性别|年龄|学历|籍贯|婚育|目前|手机|电话|邮箱|现居|居住地|工作|求职|期望|政治|民族|出生年月|出生日期|出生时间|生日|出生)[\s:：]|$)/) || nameFromFilename(filename) || '';
...
// 出生年份合理性：在职人员一般 16~70 岁，2025/2026 这种明显不是出生年份
function isValidBirthYear(year) {
  const y = Number(year);
  if (!y || y < 1900) return false;
  const now = new Date().getFullYear();
  return y <= now - 16 && y >= now - 70;
}
...
function birthYearOf(entry) {
  ...
  if (m && isValidBirthYear(m[1])) return m[1];
  const age = Number(String(entry.age || '').replace(/[^\d]/g, ''));
  if (age >= 16 && age <= 70) return String(new Date().getFullYear() - age);
  return '';
}

// 目标文件名主体：岗位-姓名-出生年份（缺哪段就用"未知"补全，确保三段都有）
function resumeFileName(entry) {
  const occ = effectiveOccupation(entry) || '未知';
  const name = effectiveName(entry) || '未知';
  const year = birthYearOf(entry) || '未知';
  return [occ, name, year].join('-');
}

// 检查 entry 是否有字段是"未知"占位，或姓名明显被岗位词污染（如"技术总监"当姓名）→ 需要 AI 重新抽取补全
function hasUnreliableFields(entry) {
  const name = effectiveName(entry);
  const occ = effectiveOccupation(entry);  const year = birthYearOf(entry);
  if (!name || name === '未知' || isJobWord(name)) return true;
  if (!occ || occ === '未知') return true;
  if (!year || year === '未知') return true;
  return false;
}
...
// 姓名和岗位都必须有（允许年份未知）
function hasNameAndOcc(entry) {
  const name = effectiveName(entry);
  const occ = effectiveOccupation(entry);
  return !!name && name !== '未知' && !!occ && occ !== '未知';
}
...
function entryNameable(entry) {
  const name = effectiveName(entry);
  const occ = effectiveOccupation(entry);
  // 姓名和岗位全空→无法生成任何有意义的名字（包括"未知"）
  if (!name && !occ) return false;
  // 姓名里带文件名连接符/路径分隔符，说明很可能是“职位-姓名”这种没拆开的占位名
  if (name && /[-_|/\\]/.test(name)) return false;
  // 姓名里混入数字也视为不可靠（"未知"没有数字）
  if (name && /\d/.test(name)) return false;
  // 纯中文短名且不像常见姓氏开头且不是"未知"→不可靠，交 AI
  if (name && /^[\u4e00-\u9fa5·]{2,4}$/.test(name) && name !== '未知' && !isLikelyChineseName(name)) return false;
  // 职位是“分类 + 相关”这种占位时不改，等 AI 补全
  if (entry.category && occ && occ === sanitizeNamePart(entry.category) + '相关') return false;
 ...
 */
async function renameResumeFile(filePath, entry, { resumeId = null, rebuildMd = true } = {}) {
  ...
  const hasUnknown = hasUnreliableFields(entry);

  let target;
  if (canName) {
    target = resumeFileName(entry);
    if (!target) return null;
  } else if (hasUnknown && hasNameAndOcc(entry)) {
    // 姓名+岗位都有，只是年份未知 → 允许用"未知"补年份
    target = resumeFileName(entry);
    if (!target) return null;
  ...
  // 安全阀：正常文件名只允许 3 段（岗位-姓名-出生年份），超过 3 段说明是垃圾名，禁止越改越长
  if (String(target).split('-').filter(Boolean).length > 3) {
    return null;
  }
  ...
  // 文件可能被 WPS/Office 占用（EBUSY），失败自动重试几次再放弃
  let renamed = false;
  let lastErr = null;
  ...
  if (!renamed) {
    console.error('[rename] 失败:', oldAbs, '→', newPath, lastErr && lastErr.message);
    recordRename({ oldPath: oldAbs, newPath, status: 'failed', error: lastErr && lastErr.message });
    return null;
  }
...
async function processFile(filePath, { force = false, multiAgent = false } = {}) {
  ...
  try {
    ...
    let birth = extractBirth(text, sourceName);
    const edu = detectEducation(text);

    // AI 返回的出生年份/年龄优先：补上正则没算出来的年份
    const applyAiBirth = (ai) => {
      if (ai && ai.birth_year && isValidBirthYear(ai.birth_year)) {
        birth.birth_date = `${ai.birth_year}年`;
      }
      if (ai && ai.age && !birth.age) birth.age = String(ai.age);
    };
    ...
    // 2. 正则不完整，或任何字段缺失/占位（将显示"未知"）→ 有 AI 时立即交给 AI 补全
    if ((!entryNameable(fullEntry) || hasUnreliableFields(fullEntry)) && db.listAPIs().length > 0) {
      try {
        const ai = multiAgent
          ? await llm.categorizeResumeMultiAgent(text, sourceName)
          : await llm.categorizeResume(text, sourceName);
    ...
    // 非简历隔离：AI 在场、内容不像简历、文件名也不像简历，才搬入「简历/未知/」文件夹
    if ((!entryNameable(fullEntry) || hasUnreliableFields(fullEntry))
        && db.listAPIs().length > 0
        && !looksLikeResume(text)
        && !looksLikeResumeFilename(sourceName)) {
      const ext = path.extname(sourceName);
      const base = path.basename(sourceName, ext);
      let dest = path.join(UNKNOWN_DIR, sourceName);
      let k = 1;
      while (fs.existsSync(dest)) {
        dest = path.join(UNKNOWN_DIR, `${base}(${k})${ext}`);
        k++;
      }
      try {
        fs.renameSync(abs, dest);
      } catch (e) {
        console.error('[未知隔离] 移动失败:', abs, e.message);
      }
      if (fs.existsSync(dest) && !fs.existsSync(abs)) {
        const oldRow = db.getResumeByPath(abs);
        if (oldRow) db.deleteResume(oldRow.id);
        db.deleteProcessedFile(abs);
        db.removeProblemFile(abs);
        db.markProcessed(dest, { hash, mtime: stat.mtimeMs, status: 'non_resume', error: '', resume_id: null });
        console.log('[未知隔离]', abs, '→', dest);
        recordRename({ oldPath: abs, newPath: dest, status: 'renamed' });
        return { ok: true, movedToUnknown: true, name: entry.name, category: entry.category, resumeId: null, renamed: true, needsAI: 1, finalPath: dest };
      }
    }

    // 只要有字段是"未知"，或正则结果不可靠（如杭州当姓名），就标记 needs_ai=1，持续交给 AI 修正
    const needsAI = (!entryNameable(fullEntry) || hasUnreliableFields(fullEntry)) ? 1 : 0;
    fullEntry.needs_ai = needsAI;
    resumeId = db.upsertResume(fullEntry);
    if (needsAI) db.upsertProblemFile(abs, '未知/不可靠，待 AI 补全');
    else db.removeProblemFile(abs);
    // 第一步：先给原文件重命名「岗位-姓名-出生年份」（规则/AI 能搜到姓名和岗位就改；
    // 年份缺失就省略；搜不到的占位数据被 entryNameable 拦截，保持原名并靠 needs_ai 等 AI 补全）
    const shouldRename = needsRename(abs, fullEntry);
    const renamed = await renameResumeFile(abs, fullEntry, { resumeId, rebuildMd: false });
    if (!renamed && shouldRename) {
      // 改名失败（如 EBUSY 文件被占用）：标记 error，交给上层稍后自动重试
      db.markProcessed(abs, { hash, mtime: stat.mtimeMs, status: 'error', error: 'rename_failed' });
      return { ok: false, error: 'rename_failed', retry: true, finalPath: abs };
    }
    const finalPath = renamed ? renamed.newPath : abs;
    // 同步问题文件登记：AI 修好且格式 OK → 删除登记；仍异常 → 登记在最终路径上
    if (needsAI) {
      db.upsertProblemFile(finalPath, '未知/不可靠，待 AI 补全');
      db.removeProblemFile(abs);
    } else {
      db.removeProblemFile(abs);
      db.removeProblemFile(finalPath);
    }
    ...
    return { ok: true, name: entry.name, category: entry.category, resumeId, renamed: !!renamed, needsAI, finalPath };
  } catch (e) {
