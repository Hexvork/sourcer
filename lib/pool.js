...
function nameFromFilename(filename) {
  ...
  for (const s of segments) {
    ...
    if (m && !isJobWord(m[1]) && !/先生|女士|小姐|老师$/.test(m[1])) {
      candidates.push(m[1]);
    }
  }
...
function effectiveOccupation(entry) {
  const fromFile = cleanOccupation(occupationFromFilename(entry.source_file));
  const direct = cleanOccupation(entry.occupation);
