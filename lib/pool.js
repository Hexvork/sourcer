...
const INVALID_NAME_WORDS = new Set(['未提供', '无', '未知', '暂无', '待定', '不详', '名称', '姓名', '不祥', '本科', '硕士', '博士', '大专', '中专', '高中', '研究生', '学士']);
...
function cleanOccupation(occ) {
  ...
  s = s
    ...
    .trim();
  // 中文岗位后跟英文缩写/昵称：人力资源总监hrd → 人力资源总监
  const enTail = s.match(/^([\u4e00-\u9fa5]{2,20})[a-zA-Z]+$/);
  if (enTail) s = enTail[1];
  if (INVALID_OCCUPATION_WORDS.has(s)) return '';
  // 历史垃圾岗位：整段标签/期望/薪资/地点/日期范围等都不是岗位
  if (/[：:]/.test(s)) return '';
  if (/期望|薪资|工作性质|工作地点|目标职位|月薪|全职|兼职|至今/.test(s)) return '';
...
const COMMON_SURNAMES = new Set(('李王张刘陈杨赵黄周吴徐孙胡朱高林何郭马罗梁宋郑谢韩唐冯于董萧程曹袁邓许傅沈曾彭吕苏卢蒋蔡贾丁魏薛叶阎余潘杜戴夏钟汪田任姜范方石姚谭廖邹熊金陆郝孔白崔康毛邱秦江史顾侯邵孟龙万段钱汤尹黎易常武乔贺赖龚文').split(''));
