...
function fallbackCategorize(text, filename) {
  ...
  const age = pickRegex(t, /(?:年龄)[\s:：]*(\d{2,3})/) || pickRegex(t, /(\d{2,3})\s*岁/) || (() => {
    const m = String(t).match(/(?:出生年月|出生日期|生日)[\s:：]*(\d{4})/);
    return m ? String(new Date().getFullYear() - Number(m[1])) : '未知';
  })();
  const educationRaw = pickRegex(t, /((?:统招|全日制)?(?:本科|硕士|博士|研究生|大专|中专|高中|MBA|EMBA))/) || pickRegex(t, /(?:学历|教育背景|最高学历)[\s:：]*([^\s,，。；;\n]{1,20})/);
  const education = hasCn(educationRaw) ? educationRaw : '未知';
