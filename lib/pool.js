...
function fallbackCategorize(text, filename) {
  ...
  const occupation = pickRegex(t, /(?:求职意向|应聘|岗位|职位|职业)[\s:：]*([^\s,，。；;\n]{2,30})/) || category.replace('岗位', '') + '相关';
  const experience = pickRegex(t, /(?:工作经历|经历|经验)[:：]?([\s\S]{0,120}?)(?=\n\s*(?:教育|项目|技能|自我|$))/i) || '';
  const other = '';
  const summary = `${name}，${gender}，${age || '年龄未知'}，${occupation}。` + (experience ? ` 经历：${experience.slice(0, 80)}` : '');
  return { category, name, gender, age, occupation, experience, university, other, summary, fallback: true };
}
...
function buildEntryBlock(entry) {
  ...
  lines.push(`- **职业**：${entry.occupation}`);
