...
function fallbackCategorize(text, filename) {
  ...
  const name = pickRegex(t, /(?:姓名|名字)[\s:：]{1,}([^\s,，。；;\n]{2,20}?)(?=\s*(?:性别|年龄|学历|籍贯|婚育|目前|手机|电话|邮箱|现居|居住地|工作|求职|期望|政治|民族|出生)[\s:：]|$)/) || nameFromFilename(filename) || filename.replace(/\.[^.]+$/, '');
  ...
  const occupation = pickRegex(t, /(?:职位名称|求职意向|应聘|岗位|职位|职业|职务)[\s:：]{1,}([^\s,，。；;\n]{2,30}?)(?=\s*(?:期望薪资|期望|薪资|姓名|性别|年龄|学历|籍贯|婚育|目前薪酬|目前|手机|电话|邮箱|现居|居住地|工作经历|求职意向|政治面貌|政治|民族|出生)[\s:：]|$)/)
...
const INVALID_NAME_WORDS = new Set(['未提供', '无', '未知', '暂无', '待定', '不详', '名称', '姓名', '不祥']);
const INVALID_OCCUPATION_WORDS = new Set(['未提供', '无', '未知', '暂无', '待定', '不详', '名称', '姓名', '岗位', '职位', '职业', '职务', '相关', '未命名']);
...
function cleanOccupation(occ) {
  ...
  s = s
    ...
    .replace(/^cho$/i, 'CHO')
    .trim();
  return INVALID_OCCUPATION_WORDS.has(s) ? '' : s;
}
...
function nameFromFilename(filename) {
  ...
  const honorific = segments.find(s => /^[\u4e00-\u9fa5]{1,2}(?:先生|女士|小姐)$/.test(s));
  return honorific || '';
}
...
function occupationFromFilename(filename) {
  ...
  const pool = jobCandidates.length ? jobCandidates : candidates;
  // 优先短的、像岗位的段，避免“速聘专猎推荐报告”这类说明性后缀
  return pool.slice().sort((a, b) => a.length - b.length || a.localeCompare(b, 'zh-CN'))[0];
}
...
function renameResumeFile(filePath, entry, { resumeId = null, rebuildMd = true } = {}) {
  ...
  // 数据不足以支撑命名（姓名/岗位为占位、含职位连接符等）时不改名，防止污染文件名
...
async function processFile(filePath, { force = false, multiAgent = false } = {}) {
  ...
  try {
    ...
    // AI 也可能抽错（如“赵亚洲性别：男”“人工智能相关”），这里用规则/文件名再做一次清洗兜底
    // 注意：只取数据库需要的字段，不要把 llm 返回的 apiName/model 带进 upsertResume
    const safeEntry = {
      name: effectiveName({ name: entry.name, source_file: sourceName }),
      occupation: effectiveOccupation({ ...entry, source_file: sourceName }),
