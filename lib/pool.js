...
const RESUME_DIR = path.join(ROOT, '简历');
const POOL_DIR = path.join(ROOT, '简历池');
const ORIGINAL_DIR = path.join(POOL_DIR, '原始文件');
...
function safeFilename(filePath, existsFn) {
...
function moveIntoPool(filePath, category) {
  const catDir = path.join(ORIGINAL_DIR, sanitizeDir(category));
  fs.mkdirSync(catDir, { recursive: true });
  const filename = safeFilename(filePath, (f) => fs.existsSync(path.join(catDir, f)));
...
function sanitizeDir(name) {
  return String(name || '其他岗位').replace(/[\\/:*?"<>|]/g, '_');
}
...
function fallbackCategorize(text, filename) {
  ...
  const age = pickRegex(t, /(?:年龄)[\s:：]*(\d{2,3})/) || pickRegex(t, /(\d{2,3})\s*岁/) || '未知';
  const education = pickRegex(t, /(?:学历|教育背景|最高学历)[\s:：]*([^\s,，。；;\n]{1,20})/) || pickRegex(t, /((?:统招|全日制)?(?:本科|硕士|博士|研究生|大专|中专|高中|MBA|EMBA))/) || '未知';
  const university = pickRegex(t, /([\u4e00-\u9fa5]{2,20}(?:大学|学院))/) || pickRegex(t, /(?:大学|学院|学校)[\s:：]*([^\s,，。；;\n]{2,20})/) || '未知';
  const occupation = pickRegex(t, /(?:求职意向|应聘|岗位|职位|职业|职务)[\s:：]*([^\s,，。；;\n]{2,30})/) || category.replace('岗位', '') + '相关';
  const company = pickRegex(t, /(?:主要任职公司|任职公司|工作单位|当前公司|公司名称)[\s:：]*([^\s,，。；;\n]{2,30})/) || pickRegex(t, /(?:曾任职于|就职于|任职于|在)([^\s,，。；;\n]{2,20}(?:公司|集团|科技|有限|股份|银行|医院))/) || '未知';
  const experience = pickRegex(t, /(?:工作经历|经历|经验)[\s:：]*([^\n]{1,200})/) || '';
  const other = '';
  const summary = `${name}，${gender}，${age || '年龄未知'}，${education}，${occupation}。` + (experience ? ` 经历：${experience.slice(0, 80)}` : '');
  return { category, name, gender, age, education, occupation, company, experience, university, other, summary, fallback: true };
}

function detectCategory(t) {
  const rules = [
    ['人工智能岗位', /人工智能|AI\s*(算法|工程师|研究员)|算法工程师|机器学习|深度学习|大模型|NLP|计算机视觉|CV|推荐算法|数据挖掘|AIGC|智能问答|语音识别/i],
    ['机器人岗位', /机器人|ROS|机械臂|运动控制|SLAM|自动驾驶|无人驾驶|具身智能|伺服|路径规划/i],
    ['数据岗位', /数据分析|数据科学|大数据|数据仓库|ETL|BI|商业智能|爬虫/i],
    ['软件技术岗位', /软件|后端|前端|Java|Python|C\+\+|Golang|Go|Node\.js|PHP|架构师|测试|运维|DevOps|嵌入式|单片机|小程序|Android|iOS|Flutter/i],
    ['硬件技术岗位', /硬件|电路|PCB|FPGA|芯片|模拟|射频|Layout|结构设计|模具/i],
    ['销售岗位', /销售|客户经理|商务|渠道|大客户|BD|拓展|招商|导购|零售/i],
    ['市场营销岗位', /市场|营销|品牌|公关|新媒体|文案|策划|活动执行|增长|投放/i],
    ['财务岗位', /财务|会计|出纳|审计|税务|成本|资金|风控|合规/i],
    ['人力资源岗位', /人力资源|HR|招聘|薪酬|绩效|培训|员工关系|行政/i],
    ['设计岗位', /设计|UI|UX|视觉|交互|平面|室内|工业设计|原画|美工/i],
    ['运营岗位', /运营|产品经理|项目管理|PM|客服|电商|供应链|物流|采购/i],
    ['医疗岗位', /医生|护士|临床|医药|医疗|护理|药|医师|康复/i],
    ['教育岗位', /教师|老师|讲师|培训|教研|课程|教务/i],
    ['法务岗位', /法务|律师|法律|合规|知识产权|专利/i],
    ['管理岗位', /总经理|总监|经理|主管|COO|CEO|CTO|CFO|副总裁|合伙人/i],
    ['其他岗位', /其他/]
  ];
...
function buildEntryBlock(entry) {
  ...
  lines.push(`- **当前文件**：${entry.pool_path}`);
...
async function processFile(filePath, { force = false } = {}) {
  ...
  try {
    ...
    let entry;
    try {
      const ai = await llm.categorizeResume(text, path.basename(abs));
    ...
    let poolPath;
    try {
      poolPath = moveIntoPool(abs, entry.category);
    } catch (e) {
      poolPath = abs;
    }
    const fullEntry = {
