...
// ---------------- 多 Agent 并发搜寻 ----------------
/**
 * 多 Agent 并发搜寻：将简历池按候选人（## 姓名 块）拆分，
 * 每个 Agent 评估一部分简历，最后由主 Agent 核查汇总。
 */
async function searchResumesMultiAgent(query, markdown, profile) {
  const extra = profileBlock(profile);

  // 1. 拆分简历池为单个候选人条目
  const entries = splitMarkdownEntries(markdown);
  if (entries.length === 0) {
    return { results: [], apiName: '', model: '', multiAgent: true };
  }

  // 2. 并发 Agent：每个子 Agent 评估若干简历
  ...
  for (const batch of batches) {
    const batchPromises = batch.map(async (entryText) => {
      const sys = '你是一名资深猎头，负责评估一份简历与岗位需求的匹配度。只输出 JSON，不要多余文字。'
        + (extra ? '\n\n' + extra : '');
      const user = `顾客岗位需求：
${query}

请评估下面这份简历的匹配度（0-100 整数）。如果匹配度 >= 70，输出 score 和 reason；否则输出 null。

简历：
${truncate(entryText, 4000)}

严格输出 JSON（不要代码块）：
{"score": 85, "reason": "该候选人有3年大模型经验，熟悉RAG和微调，与岗位高度匹配。"}
或（不匹配时）：
null`;
      ...
      try {
        ...
        const data = parseJsonLoose(content);
        if (data && data.score && data.score >= 70) {
          return { score: data.score, reason: data.reason || '', apiName, model };
        }
        return null;
      } catch (e) {
    ...
    // 提取每个结果对应的简历名称（从 entry 中提取）
    batch.forEach((entry, idx) => {
      const nameMatch = entry.match(/^##\s*([^（\n]+)/);
      const name = nameMatch ? nameMatch[1].trim() : '未知';
      const result = batchResults[idx];
      if (result) {
        subAgentResults.push({ name, ...result });
      }
    });
  }

  // 3. 主 Agent 核查：对初筛结果进行核查和排序
...
以下是各子 Agent 初筛结果（候选人与匹配度）：
${subAgentResults.map(r => `- ${r.name}：匹配度 ${r.score}，理由：${r.reason || '无'}`).join('\n')}

请做以下工作：
1. 核查每个候选人的匹配度是否合理，如有偏差请修正（0-100 整数）
2. 按匹配度从高到低排序
3. 只保留匹配度 >= 70 的候选人
4. 对每个候选人给出最终的理由（50 字以内，具体清晰）
    ...
    if (data && Array.isArray(data.results)) {
      mainResult = {
        results: data.results.map(r => ({
          ...
          reason: String(r.reason || ''),
          category: String(r.category || '')
        })).filter(r => r.name && r.score >= 70).sort((a, b) => b.score - a.score)
      };
    }
